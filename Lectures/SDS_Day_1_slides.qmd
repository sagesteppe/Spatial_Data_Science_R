...
![A GIS (Anne Sexton)](../pictures/USGS_GIS_Anne_Sexton.jpg){
  height=550 
  fig-align='right' 
}
:::
...
![The Broad Street Pump, A Cholera Outbreak in London. (John Snow)](../pictures/John_Snow_Cholera_by_National_Geographic.jpg){
  width=500 
  fig-align='right' 
}
:::
...
![Blue Marble 2012 (Suomi NPP)](../pictures/NASA_Blue-Marble_2012_Suomi_NPP.jpg){
  width=1000 
  fig-align='right' 
}
:::
...
::: {style="float: right; width: 50%; font-size: 0.8em; color: #cbc4c3; text-align: center;"}
```{r, out.width = "35%", fig.show='hold', echo = F, fig.align="center"}
...
::: {style="float: right; width: 30%; font-size: 0.8em; color: #cbc4c3; text-align: center;"}
```{r, out.width = "50%", fig.show='hold', echo = F, fig.align="center", fig.cap = "Number one City Datum (Cosmo1976)", fig.asp=0.50}
...
- Z = Elevation/Depth (somewhat uncommon)  
...
## sf 3 - Points are/form a SF Geometry (sfg's)
...
```{r sf 3 - Points are/form a SF Geometry (sfg), echo = F}
...
```{r sf 4 - Geometries can form Geometries (sfg), echo = F}
...
- But *why* are you doing this? Why should the viewer focus on this? 

```{r sf with ggplot2 - 4, out.width = "75%", fig.show='hold', fig.align="center"}
...
```{r sf with ggplot2 - 5a, out.width = "75%", fig.show='hold', fig.align="center"}
...
```{r sf with ggplot2 - 5b, out.width = "75%", fig.show='hold', fig.align="center"}
#| code-line-numbers: "2-5"
ggplot(north_carolina) +
  geom_sf(
    aes(fill = BIR79),
  ...
  scale_color_viridis_c(
    option = "plasma", 
    trans = "sqrt" 
    # some data have a couple very large
    # values which can swamp the palettes
    ) +
  theme_bw()
```
...
- fill the interior of polygons and color the borders

```{r sf with ggplot2 - 6, out.width = "75%", fig.show='hold', fig.align="center"}
#| code-line-numbers: "2-5"
ggplot(north_carolina) +
  geom_sf(
    aes(fill = BIR79, 
...
```{r sf with ggplot2 - 7, out.width = "75%", fig.show='hold', fig.align="center"}
ggplot() + # leave empty
  geom_sf(data = north_carolina, # note need to specify the data = object. 
          aes(fill = BIR79, 
              color = FIPS)
          ) + # both fill and color
  guides(color = 'none') + # removed the categorical legend (is too big!)
...
```{r sf with ggplot2 - 8, out.width = "75%", fig.show='hold', fig.align="center"}
#| code-line-numbers: "2-5"
ggplot(us_states) + # two data sets.
  geom_sf() + # without specifying data, will inherit from plot call 
...
```{r sf with ggplot2 - 9, out.width = "75%", fig.show='hold', fig.align="center"}
#| code-line-numbers: "3"
...
```{r sf with ggplot2 - 10, out.width = "75%", fig.show='hold', fig.align="center"}
...
```{r sf with ggplot2 - 11, out.width = "75%", fig.show='hold', fig.align="center"}
...
```{r sf with ggplot2 - 12, out.width = "75%", fig.show='hold', fig.align="center"}
#| code-line-numbers: "7-10"

bound <- st_bbox(north_carolina) # retrieve the bounding box from the sfc list column
