...
Schema for describing spatial relationships  
...
```{r Plot Topology, echo = F, fig.cap = "Spatial Predicates after Egenhofer and Herring (1990)", fig.align="center", out.height="25%"}
...
```{r Spatial Joins 2, comment = "", message = F, warning = F, echo = F, out.height="25%"}
...
## Make a geometry valid as a Simple Feature I 

- Some data may not be in a simple format
...
## Make a geometry valid as a Simple Feature II 

- Polygon became a multipolygon with two polygons
- Removed the self-intersection at position X5, Y5. 
...
- Determine the interior area of a polygon. 
...
```{r Area - 6, message = F, echo = F, fig.align="center", warning = F}
...
- calculate exterior using `st_length` 
...
- If the centroid would fall outside of the polygons interior, use `st_point_on_surface` to constrain it   

```{r Centoid and POS, message = F, warning = F, echo = TRUE}
chi_cent <- st_centroid(chi_neighb)
chi_pos <- st_point_on_surface(chi_neighb)
```

```{r Centroid and POS Plot, echo = F, fig.align="center", out.width = "75%", warning = F}
...
```{r Neighborhood Centroids, out.width="50%"}
...
```{r Cast 3, message = F, echo = F, out.width = "25%", fig.align="center"}
...
```{r, out.width = "35%", fig.align="center", fig.show='hold', echo = F, fig.cap = "Great Circle Distance (ChecCheDaWaff)"}
...
- Find the areas which are closest to objects
...
```{r Create Voronoi Polygons, echo = F, warning = F}

dat2 <- dat1 %>% 
  filter(!individual %in% c(13, 43)) %>% 
  st_as_sf(coords = c(y = 'longitude', x ='latitude'), crs = 32610) 

pols <- st_collection_extract(st_voronoi(do.call(c, st_geometry(dat2))))
pols <- pols %>% st_as_sf(crs = 32610) 

res <- st_intersects(dat2, pols) %>% 
  unlist()

pols <- pols %>% mutate(pt = res) 
pols$area <- st_area(pols) %>% 
  as.numeric()

ggplot() +
  geom_sf(data = pols, aes(fill = area), colour = 'grey60') +
  geom_sf(data = dat2, shape = 19, colour = 'gold1') +
  coord_sf(ylim = c(4527046, 4527830), xlim = c(403150,403700))  +
  labs(fill = bquote("Area"~ m^2), title = "Areas which are closest to Observations; Voronoi Polygons") +
  theme(plot.title = element_text(hjust = 0.5),
