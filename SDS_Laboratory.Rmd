...
```{r Search for shapefiles using list.files, warning = F}
...
```{r Combine our paths and file names using paste}
...
```{r Import our study site vector data}
...
```{r determine whether our vector data are valid simple features, comment = ""}
st_is_valid(sites) 
```
...
```{r check CRS of our vector data, comment = ""}
st_crs(sites)
```
...
```{r visually inspect the sites with a map 1}
sites %>% 
...
```{r visually inspect the sites with a map 2}
sites %>% 
  group_by(SITE) %>% # we want to see our SITES
  slice(1) %>% # there are several PLOTS per SITE, grab one
  st_buffer(100) %>% # we will put a 200 meter diameter circle around it
...
```{r Create an interactive map, eval = F}
leaflet(data = sites) %>% 
  addTiles() %>%
...
```{r Join tabular data to vector data}
...
```{r Collect URLs for Raster Products}
sdp_prods <- 
...
```{r Explore the Spatial Data Products, comment = ""}
...
```{r access landcover raster data, comment = ""}
...
```{r, access landcover metadata, eval = F, comment = ""}
...
```{r access Summer Solar Radiation}
...
```{r Create a raster stack of the Upper Gunnison Rasters, comment = ""}
...
```{r Reclassify Values in our vector file}
...
```{r Identify candidate sites across rasters}
...
```{r create a copy of the cells again}
...
```{r join our empty raster with the suitable cells}
...
```{r Populate the Predicted Raster with Values}
...
```{r Create a masked copy of our predictions Raster}
...
```{r create a vector copy of our predictions}
...
```{r ensure conversion is valid, eval = F}
...
```{r Inspect Raster Data for a Map}
...
```{r how many 1x1 meters cell sites do we have, comment = ""}
...
```{r Retrieve stamen map, cache = T, comment = ""}
site_bbox <- sites %>% 
  st_buffer(250) %>% 
  st_transform(4326) %>% 
  st_bbox(sites)
...
```{r write out prediction in vector format, eval = F, comment = ""}
...
```{r write out prediction in raster format, comment = ""}
raster::writeFormats()
...
```{r Create Vector Data from Coordinates in CSV, eval = F}
...
```{r elevation data, eval = F, echo = F}
...
```{r Access Slope Angle, eval = F, echo = F}
...
```{r Access Topo Southness, eval = F, echo = F}
...
```{r Access Topo Westness, eval = F, echo = F}
...
```{r Solar radiation, eval = F, echo = F}
...
```{r extract 100 random points to match the study sites, eval = F}
